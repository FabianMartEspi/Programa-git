...
function showLoadingScreen($loadingScreen, duration, callback) {
...
// Método para crear una pestaña
function createTabContent(navId, autoOpen, config) {
  if (config.loadingScreen) {
    var $loadingScreen = $(SELECTOR_TAB_LOADING);
    showLoadingScreen($loadingScreen, config.loadingScreen, function() {
      switchTab("#" + navId);
    });
  } else {
    switchTab("#" + navId);
...
// Método para abrir la barra lateral de pestañas
_proto.openTabSidebar = function openTabSidebar(item, autoOpen = this._config.autoShowNewTab) {
  var $item = $(item).clone();
  ...
  var title = $item.find('p').text() || $item.text();
  ...
  if (!this._config.allowDuplicates && $("#" + navId).length > 0) {
    return switchTab("#" + navId, this._config.allowReload);
  }
...
// Código principal para la lógica de autoapertura
if (autoOpen) {
  createTabContent(navId, autoOpen, this._config);
}
